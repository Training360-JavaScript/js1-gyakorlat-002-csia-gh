...
const checker = (arr, val) => {
	return {
		exists: arr.includes(val),
		index: arr.includes(val) ? arr.indexOf(val) : -1,
		allElementsAreNumbers: arr.every((el) => typeof el === "number"),
		someElementsAreNumbers: arr.some((el) => typeof el === "number"),
	};
};
